...
class DarkSunCalendar {
  ...
  async initialize() {
    ...
    if (!game.seasonsStars?.api) {
      console.error(
        "🌞 Dark Sun Calendar | ERROR: Seasons & Stars is not loaded! This module requires Seasons & Stars to function."
      );
    ...
    console.log(
      "🌞 Dark Sun Calendar | Loading local dark-sun.json configuration..."
    );
    ...
    if (!activeCalendar) {
      console.error(
        "🌞 Dark Sun Calendar | ERROR: No active calendar found in Seasons & Stars!"
      );
    ...
    if (activeCalendar.id !== "dark-sun") {
      console.log(
        "🌞 Dark Sun Calendar | Setting Dark Sun as active calendar..."
      );
      try {
        await this.seasonsStars.manager.setActiveCalendar("dark-sun");
        console.log(
          "🌞 Dark Sun Calendar | Successfully set Dark Sun as active calendar"
        );
      } catch (error) {
        console.error(
          "🌞 Dark Sun Calendar | ERROR: Failed to set Dark Sun as active calendar:",
          error
        );
        throw new Error(
          `Failed to activate Dark Sun calendar: ${error.message}`
        );
      }
    }
    ...
    if (
      finalActiveCalendar &&
      finalActiveCalendar.year &&
      finalActiveCalendar.year.currentYear
    ) {
      console.log(
        `🌞 Dark Sun Calendar | Using calendar with currentYear: ${finalActiveCalendar.year.currentYear}`
      );
      if (finalActiveCalendar.year.currentYear === 14579) {
        console.log(
          "🌞 Dark Sun Calendar | ✅ Confirmed using local dark-sun.json configuration"
        );
      } else if (finalActiveCalendar.year.currentYear === 102) {
        console.log(
          "🌞 Dark Sun Calendar | ✅ Confirmed using Seasons & Stars built-in Dark Sun calendar"
        );
      } else {
        console.warn(
          `🌞 Dark Sun Calendar | ⚠️ Unexpected calendar currentYear: ${finalActiveCalendar.year.currentYear}`
        );
        console.warn(
          "🌞 Dark Sun Calendar | Expected 14579 (local) or 102 (S&S built-in)"
        );
      }
    }
  ...
  async loadDarkSunCalendar() {
    try {
      ...
      console.log(
        "🌞 Dark Sun Calendar | Attempting to load local dark-sun.json configuration..."
      );
      const response = await fetch(
        "modules/dark-sun-calendar/calendar/dark-sun.json"
      );
      ...
      if (
        !calendarData.intercalary ||
        !Array.isArray(calendarData.intercalary)
      ) {
        throw new Error("Calendar data missing or invalid intercalary array");
      }

      if (
        !calendarData.extensions ||
        !calendarData.extensions["seasons-and-stars"]
      ) {
      ...
      for (let i = 0; i < calendarData.months.length; i++) {
        ...
        if (!month.name || typeof month.days !== "number" || month.days <= 0) {
          throw new Error(
            `Invalid month data at index ${i}: missing name or invalid days`
          );
        }
      }
      ...
      for (let i = 0; i < calendarData.intercalary.length; i++) {
        ...
        if (
          !inter.name ||
          typeof inter.days !== "number" ||
          inter.days <= 0 ||
          typeof inter.after !== "number"
        ) {
          throw new Error(
            `Invalid intercalary data at index ${i}: missing name, invalid days, or missing after property`
          );
        }
      }

      console.log(
        "🌞 Dark Sun Calendar | Successfully loaded and validated local dark-sun.json"
      );
      ...
      if (!success) {
        throw new Error(
          "Failed to register local calendar with Seasons & Stars"
        );
      }

      console.log(
        "🌞 Dark Sun Calendar | ✅ Successfully registered local Dark Sun calendar with Seasons & Stars"
      );
    } catch (error) {
      console.warn(
        "🌞 Dark Sun Calendar | ⚠️ Failed to load local dark-sun.json, falling back to S&S built-in:",
        error.message
      );
      ...
      const existingDarkSun = existingCalendars.find(
        (cal) => cal.id === "dark-sun"
      );

      if (existingDarkSun) {
        console.log(
          "🌞 Dark Sun Calendar | ✅ Using Seasons & Stars built-in Dark Sun calendar as fallback"
        );
      } else {
        // Last resort: S&S doesn't have Dark Sun calendar either
        console.error(
          "🌞 Dark Sun Calendar | ERROR: No Dark Sun calendar found in local or S&S configurations"
        );
  ...
  async initializeMoonSystem() {
    ...
    try {
      ...
      console.log(
        "🌞 Dark Sun Calendar | ✅ Athasian Moon System initialized successfully"
      );
      console.log(
        `🌞 Dark Sun Calendar | Eclipse cycle: ${this.moonSystem.eclipseCycle} days`
      );
      ...
      console.log(
        `🌞 Dark Sun Calendar | Current moons (Absolute Day ${currentAbsoluteDay}):`,
        {
      ...
      const currentEclipse = this.eclipseCalculator.getEclipseInfo(
        currentAbsoluteDay
      );
      if (currentEclipse.type !== "none") {
        console.log(
          `🌞 Dark Sun Calendar | Current eclipse:`,
          currentEclipse.description
        );
      }
    } catch (error) {
      console.error(
        "🌞 Dark Sun Calendar | ERROR: Failed to initialize moon system:",
        error
      );
  ...
  getCurrentDate(...args) {
    ...
    if (this._lastIntercalaryDate && this._lastIntercalaryDate.isIntercalary) {
      console.log(
        `🌞 Dark Sun Calendar | Returning stored intercalary date:`,
        this._lastIntercalaryDate
      );
    ...
    const dateObject = currentDate.toObject
  ...
  getSeasonInfo(date, calendarId) {
    ...
    try {
      ...
      const seasonsConfig =
        activeCalendar.extensions?.["seasons-and-stars"]?.seasons;
  ...
  getMoonPhaseInfo(date, moonName) {
    try {
      if (!this.moonSystem) {
        console.warn(
          "🌞 Dark Sun Calendar | getMoonPhaseInfo: Moon system not initialized"
        );
        return [];
      }
      ...
      if (!targetDate) {
        ...
        if (!targetDate) {
          console.warn(
            "🌞 Dark Sun Calendar | getMoonPhaseInfo: Could not get current date"
          );
          return [];
        }
      }
      ...
      const absoluteDay = window.AthasianCalendarCore.dateToAbsoluteDays(
        targetDate
      );
      ...
      const moonPhaseInfo = [
        {
          ...
          daysUntilNext:
            moonData.ral.period -
            Math.floor(moonData.ral.phase * moonData.ral.period),
          phase: moonData.ral.phase,
          description:
            "The smaller, green-yellow moon of Athas, companion to Guthay in the hostile sky",
        },
        {
          ...
          // Legacy compatibility fields
          dayInPhase: Math.floor(
            moonData.guthay.phase * moonData.guthay.period
          ),
          daysUntilNext:
            moonData.guthay.period -
            Math.floor(moonData.guthay.phase * moonData.guthay.period),
          phase: moonData.guthay.phase,
          description:
            "The larger, golden moon of Athas, burning bright in the crimson sky",
        },
      ];

      // Filter by moon name if specified
      if (moonName) {
        return moonPhaseInfo.filter(
          (info) => info.moonName.toLowerCase() === moonName.toLowerCase()
        );
      }
      ...
      console.error(
        "🌞 Dark Sun Calendar | getMoonPhaseInfo: Error calculating moon phases:",
        error
      );
      return [];
    }
  ...
  getMoonPhaseSummary(date) {
    ...
    return moonPhases.map((phase) => this.formatMoonPhase(phase)).join("\n");
  }
  ...
  getNextMoonPhase(moonName, phaseName, startDate, maxDays = 200) {
    try {
      ...
      for (let dayOffset = 1; dayOffset <= maxDays; dayOffset++) {
        // Advance to next day
        const nextDate = this.seasonsStars.manager
          .getActiveEngine()
          .addDays(currentDate, dayOffset);
      ...
      console.error(
        "🌞 Dark Sun Calendar | getNextMoonPhase: Error finding next moon phase:",
        error
      );
      return null;
    }
  ...
  getYearName(year) {
    ...
    try {
      ...
      const namedYearsConfig =
        activeCalendar.extensions?.["seasons-and-stars"]?.namedYears;
      if (
        !namedYearsConfig ||
        !namedYearsConfig.names ||
        !Array.isArray(namedYearsConfig.names)
      ) {
        return "Unknown Year";
      }
  ...
  getCalendarInfo() {
    ...
    return {
      ...
      name:
        activeCalendar.translations?.en?.label ||
        activeCalendar.name ||
        "Unknown",
      description:
        activeCalendar.translations?.en?.description ||
      ...
      hasNamedYears: !!activeCalendar.extensions?.["seasons-and-stars"]
        ?.namedYears?.names?.length,
      namedYearCount:
        activeCalendar.extensions?.["seasons-and-stars"]?.namedYears?.names
          ?.length || 0,
      hasSeasons: !!activeCalendar.extensions?.["seasons-and-stars"]?.seasons
        ?.length,
      seasonCount:
        activeCalendar.extensions?.["seasons-and-stars"]?.seasons?.length || 0,
      configSource: (() => {
  ...
  getAPI() {
    return {
      ...
      getYearFromKingsAge: (kingsAge, kingsAgeYear) =>
        this.getYearFromKingsAge(kingsAge, kingsAgeYear),
      setKingsAgeDate: (kingsAge, kingsAgeYear, month, day) =>
        this.setKingsAgeDate(kingsAge, kingsAgeYear, month, day),

      // Moon phase methods
      getMoonPhaseInfo: (date, moonName) =>
        this.getMoonPhaseInfo(date, moonName),
      ...
      getNextMoonPhase: (moonName, phaseName, startDate, maxDays) =>
        this.getNextMoonPhase(moonName, phaseName, startDate, maxDays),
      ...
      getKingsAgeFromFreeYear: (freeYear) =>
        this.getKingsAgeFromFreeYear(freeYear),
      getKingsAgeYearFromFreeYear: (freeYear) =>
        this.getKingsAgeYearFromFreeYear(freeYear),
      getYearNameFromFreeYear: (freeYear) =>
        this.getYearNameFromFreeYear(freeYear),
      formatDarkSunDateFromFreeYear: (freeYear) =>
        this.formatDarkSunDateFromFreeYear(freeYear),
      ...
      // New method
      setKingsAge: (kingsAge, kingsAgeYear, dateOptions = {}) =>
        this.setKingsAge(kingsAge, kingsAgeYear, dateOptions),
    };
  }
  ...
  formatDarkSunDate(date) {
    ...
    const kingsAge =
      dateObject.kingsAge !== undefined
        ? dateObject.kingsAge
        : this.getKingsAge(dateObject.year);
    const kingsAgeYear =
      dateObject.kingsAgeYear !== undefined
        ? dateObject.kingsAgeYear
        : this.getKingsAgeYear(dateObject.year);
    ...
    if (dateObject.calendar && dateObject.month && dateObject.calendar.months) {
      monthName =
        dateObject.calendar.months[dateObject.month - 1]?.name || "Unknown";
    ...
    let dayOrdinal = day;
    if (
      typeof this.getOrdinalSuffix === "function" &&
      typeof day === "number"
    ) {
    ...
    if (kingsAge && kingsAgeYear && yearName && monthName && day) {
      return `${kingsAgeYear}${this.getOrdinalSuffix(
        kingsAgeYear
      )} Year of King's Age ${kingsAge}, ${dayOrdinal} of ${monthName}, Year of ${yearName}`;
    }
  ...
  getYearFromKingsAge(kingsAge, kingsAgeYear) {
    ...
    if (typeof kingsAge !== "number" || kingsAge < 1) {
      console.error(
        "🌞 Dark Sun Calendar | getYearFromKingsAge: Invalid King's Age:",
        kingsAge
      );
      return 1;
    }
    ...
    if (
      typeof kingsAgeYear !== "number" ||
      kingsAgeYear < 1 ||
      kingsAgeYear > 77
    ) {
      console.error(
        "🌞 Dark Sun Calendar | getYearFromKingsAge: Invalid King's Age Year:",
        kingsAgeYear
      );
      return 1;
    }
  ...
  getFreeYear(internalYear) {
    ...
    if (internalYear === undefined || internalYear === null) {
      ...
      if (!currentDate) {
        console.error(
          "🌞 Dark Sun Calendar | getFreeYear: Could not get current date"
        );
        return -14578;
      }
    ...
    if (typeof internalYear !== "number") {
      console.error(
        "🌞 Dark Sun Calendar | getFreeYear: Invalid internal year input:",
        internalYear
      );
  ...
  getInternalYear(freeYear) {
    ...
    if (freeYear === undefined || freeYear === null) {
      ...
      if (!currentDate) {
        console.error(
          "🌞 Dark Sun Calendar | getInternalYear: Could not get current date"
        );
        return 1;
      }
    ...
    if (typeof freeYear !== "number") {
      console.error(
        "🌞 Dark Sun Calendar | getInternalYear: Invalid Free Year input:",
        freeYear
      );
  ...
  getKingsAgeFromFreeYear(freeYear) {
    ...
    if (freeYear === undefined || freeYear === null) {
      ...
      if (!currentDate) {
        console.error(
          "🌞 Dark Sun Calendar | getKingsAgeFromFreeYear: Could not get current date"
        );
        return 1;
      }
      freeYear = this.getFreeYear(currentDate.year);
  ...
  getKingsAgeYearFromFreeYear(freeYear) {
    ...
    if (freeYear === undefined || freeYear === null) {
      ...
      if (!currentDate) {
        console.error(
          "🌞 Dark Sun Calendar | getKingsAgeYearFromFreeYear: Could not get current date"
        );
        return 1;
      }
      freeYear = this.getFreeYear(currentDate.year);
  ...
  getYearNameFromFreeYear(freeYear) {
    ...
    if (freeYear === undefined || freeYear === null) {
      ...
      if (!currentDate) {
        console.error(
          "🌞 Dark Sun Calendar | getYearNameFromFreeYear: Could not get current date"
        );
        return "Unknown Year";
      }
      freeYear = this.getFreeYear(currentDate.year);
  ...
  formatDarkSunDateFromFreeYear(freeYear) {
    ...
    if (freeYear === undefined || freeYear === null) {
      ...
      if (!currentDate) {
        console.error(
          "🌞 Dark Sun Calendar | formatDarkSunDateFromFreeYear: Could not get current date"
        );
        return "Unknown Date";
      }
      freeYear = this.getFreeYear(currentDate.year);
  ...
  setKingsAgeDate(kingsAge, kingsAgeYear, month, day) {
    ...
    if (!currentDate) {
      console.error(
        "🌞 Dark Sun Calendar | setKingsAgeDate: Could not get current date"
      );
      return false;
    }
    ...
    if (newKingsAge !== undefined && newKingsAge !== null) {
      if (typeof newKingsAge !== "number" || newKingsAge < 1) {
        console.error(
          "🌞 Dark Sun Calendar | setKingsAgeDate: Invalid King's Age:",
          newKingsAge
        );
        return false;
      }
    ...
    if (newKingsAgeYear !== undefined && newKingsAgeYear !== null) {
      if (
        typeof newKingsAgeYear !== "number" ||
        newKingsAgeYear < 1 ||
        newKingsAgeYear > 77
      ) {
        console.error(
          "🌞 Dark Sun Calendar | setKingsAgeDate: Invalid King's Age Year:",
          newKingsAgeYear
        );
        return false;
      }
    ...
    if (newMonth !== undefined && newMonth !== null) {
      if (typeof newMonth !== "number" || newMonth < 1 || newMonth > 12) {
        console.error(
          "🌞 Dark Sun Calendar | setKingsAgeDate: Invalid month:",
          newMonth
        );
        return false;
      }
    ...
    if (newDay !== undefined && newDay !== null) {
      if (typeof newDay !== "number" || newDay < 1 || newDay > 30) {
        console.error(
          "🌞 Dark Sun Calendar | setKingsAgeDate: Invalid day:",
          newDay
        );
        return false;
      }
    ...
    try {
      ...
      console.error(
        "🌞 Dark Sun Calendar | setKingsAgeDate: Error setting date:",
        error
      );
      return false;
    }
  ...
  setDayOfYear(dayOfYear, year) {
    ...
    const isIntercalaryPeriod =
      (dayOfYear >= 121 && dayOfYear <= 125) ||
    ...
    if (isIntercalaryPeriod) {
      ...
      console.log(
        `🌞 Dark Sun Calendar | setDayOfYear(${dayOfYear}) - Handling intercalary period ourselves`
      );
    ...
    const {
      ...
      intercalary,
      intercalaryDay,
    } = this.dayOfYearToMonthDay(dayOfYear);

    console.log(
      `🌞 Dark Sun Calendar | setDayOfYear(${dayOfYear}) -> month: ${month}, day: ${day}, intercalary: ${intercalary}, intercalaryDay: ${intercalaryDay}`
    );
  ...
  dayOfYearToMonthDay(dayOfYear) {
    ...
    if (dayOfYear >= 121 && dayOfYear <= 125) {
      ...
      const result = {
        month: 4,
        day: 1,
        intercalary: "Cooling Sun",
        intercalaryDay: intercalaryDay,
      };
      console.log(
        `🌞 Dark Sun Calendar | Day ${dayOfYear} is Cooling Sun day ${intercalaryDay} -> ${JSON.stringify(
          result
        )}`
      );
      return result;
      ...
      const result = {
        month: 8,
        day: 1,
        intercalary: "Soaring Sun",
        intercalaryDay: intercalaryDay,
      };
      console.log(
        `🌞 Dark Sun Calendar | Day ${dayOfYear} is Soaring Sun day ${intercalaryDay} -> ${JSON.stringify(
          result
        )}`
      );
      return result;
      ...
      const result = {
        month: 12,
        day: 1,
        intercalary: "Highest Sun",
        intercalaryDay: intercalaryDay,
      };
      console.log(
        `🌞 Dark Sun Calendar | Day ${dayOfYear} is Highest Sun day ${intercalaryDay} -> ${JSON.stringify(
          result
        )}`
      );
      return result;
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      if (remainingDays <= daysInMonth) {
        ...
        console.log(
          `🌞 Dark Sun Calendar | Day ${dayOfYear} is regular month ${m}, day ${remainingDays} -> ${JSON.stringify(
            result
          )}`
        );
        return result;
    ...
    console.log(
      `🌞 Dark Sun Calendar | Day ${dayOfYear} fallback -> ${JSON.stringify(
        result
      )}`
    );
    return result;
...
Hooks.once("init", () => {
  ...
  if (!game.modules.get("seasons-and-stars")?.active) {
    console.error(
      "🌞 Dark Sun Calendar: Seasons & Stars module is not active!"
    );
    return;
  }
});
...
window.DSC = {
  ...
  getAvailableCalendars: (...args) =>
    darkSunCalendar?.getAvailableCalendars(...args),
  ...
  getMoonPhaseInfo: (date, moonName) =>
    darkSunCalendar?.getMoonPhaseInfo(date, moonName),
  ...
  getEclipseInfo: (date) => {
    ...
    const absoluteDay = window.AthasianCalendarCore?.dateToAbsoluteDays(
      date || darkSunCalendar.getCurrentDate()
    );
  ...
  findNextEclipse: (minType = "partial") => {
    ...
    return darkSunCalendar.eclipseCalculator.findNextEclipse(
      absoluteDay,
      minType
    );
  },
  findPreviousEclipse: (minType = "partial") => {
    if (!darkSunCalendar?.eclipseCalculator) return null;
    const absoluteDay = window.AthasianCalendarCore?.getCurrentAbsoluteDay();
    return darkSunCalendar.eclipseCalculator.findPreviousEclipse(
      absoluteDay,
      minType
    );
  },
  ...
  toAbsoluteDays: (kingsAge, year, dayOfYear) =>
    window.AthasianCalendarCore?.toAbsoluteDays(kingsAge, year, dayOfYear),
  fromAbsoluteDays: (absoluteDays) =>
    window.AthasianCalendarCore?.fromAbsoluteDays(absoluteDays),
  getCurrentAbsoluteDay: () =>
    window.AthasianCalendarCore?.getCurrentAbsoluteDay(),
  ...
  getNextMoonPhase: (moonName, phaseName, startDate, maxDays) =>
    darkSunCalendar?.getNextMoonPhase(moonName, phaseName, startDate, maxDays),
  ...
  // New method
  getYearFromKingsAge: (kingsAge, kingsAgeYear) =>
    darkSunCalendar?.getYearFromKingsAge(kingsAge, kingsAgeYear),
  ...
  getKingsAgeFromFreeYear: (freeYear) =>
    darkSunCalendar?.getKingsAgeFromFreeYear(freeYear),
  getKingsAgeYearFromFreeYear: (freeYear) =>
    darkSunCalendar?.getKingsAgeYearFromFreeYear(freeYear),
  getYearNameFromFreeYear: (freeYear) =>
    darkSunCalendar?.getYearNameFromFreeYear(freeYear),
  formatDarkSunDateFromFreeYear: (freeYear) =>
    darkSunCalendar?.formatDarkSunDateFromFreeYear(freeYear),

  // New method
  setKingsAgeDate: (kingsAge, kingsAgeYear, month, day) =>
    darkSunCalendar?.setKingsAgeDate(kingsAge, kingsAgeYear, month, day),

  // New method for setting date by day of year
  setDayOfYear: (dayOfYear, year) =>
    darkSunCalendar?.setDayOfYear(dayOfYear, year),
...
Hooks.once("seasons-stars:ready", async () => {
  await (window.CalendarDate
    ? Promise.resolve()
    : new Promise((resolve) => {
        const check = () =>
          window.CalendarDate ? resolve() : setTimeout(check, 50);
        check();
      }));
});
